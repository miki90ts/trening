...
async function getFoods(user, query) {
...
async function getFoodById(id, user) {
  const [rows] = await pool.query("SELECT * FROM food_items WHERE id = ?", [
    id,
  ]);
  if (rows.length === 0) {
    throw httpError(404, "Namirnica nije pronađena");
  }

  if (!rows[0].is_active && user.role !== "admin") {
    throw httpError(404, "Namirnica nije pronađena");
  }
...
async function createFood(user, body) {
  const {
    name,
    item_type = "food",
    kcal_per_100g,
  ...
  if (!name || !String(name).trim()) {
    throw httpError(400, "Naziv je obavezan");
  }

  if (!["food", "dish"].includes(item_type)) {
    throw httpError(400, "item_type mora biti food ili dish");
  }
  ...
  if (!macros) {
    throw httpError(
      400,
      "Kcal i makronutrijenti moraju biti validni brojevi >= 0",
    );
  }
  ...
  const [created] = await pool.query("SELECT * FROM food_items WHERE id = ?", [
    result.insertId,
  ]);
  return created[0];
}
...
async function deleteFood(id) {
  ...
  return { message: "Namirnica deaktivirana" };
}
