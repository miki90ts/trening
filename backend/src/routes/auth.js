...
async function verifyRefreshToken(refreshToken) {
  ...
  const [rows] = await pool.query(
    'SELECT rt.*, u.id as uid, u.email, u.role, u.is_approved, u.first_name, u.last_name, u.nickname, u.profile_image FROM refresh_tokens rt JOIN users u ON rt.user_id = u.id WHERE rt.token_hash = ? AND rt.expires_at > NOW()',
    [tokenHash]
  );
...
router.post('/register', async (req, res, next) => {
  try {
    ...
    if (!/[A-Z]/.test(password) || !/[a-z]/.test(password) || !/[0-9]/.test(password)) {
      return res.status(400).json({ error: 'Lozinka mora sadržati veliko slovo, malo slovo i broj.' });
    }
    ...
    const [result] = await pool.query(
      ...
      [first_name, last_name || null, nickname || null, email.toLowerCase(), passwordHash, 'user', 0]
    );
...
router.post('/refresh', async (req, res, next) => {
  try {
    ...
    if (!tokenData) {
      return res.status(401).json({ error: 'Nevažeći ili istekli refresh token.' });
    }
    ...
    const newRefreshToken = await saveRefreshToken(user.id, generateRefreshToken());
...
router.put('/approve/:id', authenticate, authorize('admin'), async (req, res, next) => {
  try {
    const [result] = await pool.query('UPDATE users SET is_approved = 1 WHERE id = ?', [req.params.id]);
...
router.delete('/reject/:id', authenticate, authorize('admin'), async (req, res, next) => {
  try {
    const [user] = await pool.query('SELECT * FROM users WHERE id = ? AND is_approved = 0', [req.params.id]);
    if (user.length === 0) {
      return res.status(404).json({ error: 'Korisnik nije pronađen ili je već odobren.' });
    }
...
router.put('/role/:id', authenticate, authorize('admin'), async (req, res, next) => {
  try {
    const { role } = req.body;
    if (!['user', 'admin'].includes(role)) {
      return res.status(400).json({ error: 'Rola mora biti "user" ili "admin".' });
    }

    // Ne dozvoli adminu da sam sebi skine admin prava
    if (parseInt(req.params.id) === req.user.id && role !== 'admin') {
      return res.status(400).json({ error: 'Ne možete sami sebi ukloniti admin prava.' });
    }
...
router.post('/change-password', authenticate, async (req, res, next) => {
  try {
    ...
    if (!/[A-Z]/.test(newPassword) || !/[a-z]/.test(newPassword) || !/[0-9]/.test(newPassword)) {
      return res.status(400).json({ error: 'Nova lozinka mora sadržati veliko slovo, malo slovo i broj.' });
    }
