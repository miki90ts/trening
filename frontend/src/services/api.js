...
export const logout = (token) => api.post('/auth/logout', { refreshToken: token }).then(r => r.data);
...
export const rejectUser = (id) => api.delete(`/auth/reject/${id}`).then(r => r.data);
export const changeUserRole = (id, role) => api.put(`/auth/role/${id}`, { role }).then(r => r.data);
...
export const createUser = (data) => {
  ...
  return api.post('/users', fd, { headers: { 'Content-Type': 'multipart/form-data' } }).then(r => r.data);
};
export const updateUser = (id, data) => {
  const fd = new FormData();
  Object.entries(data).forEach(([k, v]) => { if (v != null) fd.append(k, v); });
  return api.put(`/users/${id}`, fd, { headers: { 'Content-Type': 'multipart/form-data' } }).then(r => r.data);
};
export const deleteUser = (id) => api.delete(`/users/${id}`).then(r => r.data);
export const getUserRecords = (id) => api.get(`/users/${id}/records`).then(r => r.data);
...
export const createExercise = (data) => {
  const fd = new FormData();
  Object.entries(data).forEach(([k, v]) => { if (v != null) fd.append(k, v); });
  return api.post('/exercises', fd, { headers: { 'Content-Type': 'multipart/form-data' } }).then(r => r.data);
};
export const updateExercise = (id, data) => {
  const fd = new FormData();
  Object.entries(data).forEach(([k, v]) => { if (v != null) fd.append(k, v); });
  return api.put(`/exercises/${id}`, fd, { headers: { 'Content-Type': 'multipart/form-data' } }).then(r => r.data);
};
...
// ======== RESULTS ========
export const getResults = (filters = {}) => api.get('/results', { params: filters }).then(r => r.data);
export const getResultsPaginated = (params) => api.get('/results', { params }).then(r => r.data);
export const getResultDetail = (id) => api.get(`/results/${id}`).then(r => r.data);
...
export const getSummary = () => api.get('/leaderboard/summary').then(r => r.data);
