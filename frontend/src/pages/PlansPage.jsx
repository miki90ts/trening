...
function PlansPage() {
  ...
  const [tab, setTab] = useState('plans'); // 'plans' | 'sessions'
      ...
      {tab === 'plans' && (
        <div className="plans-grid">
          {plans.length === 0 ? (
            ...
            plans.map(plan => (
              <div key={plan.id} className="plan-card" style={{ borderTopColor: plan.color || '#6366f1' }}>
                <div className="plan-card-header">
                  <div className="plan-card-color" style={{ background: plan.color || '#6366f1' }} />
                ...
                <div className="plan-card-actions">
                  ...
                  <button className="btn btn-sm btn-ghost" onClick={() => navigate(`/plans/${plan.id}/edit`)} title="Izmeni">
                    <FiEdit2 />
                  </button>
                  <button className="btn btn-sm btn-ghost" onClick={() => navigate(`/plans/new?copyFrom=${plan.id}`)} title="Kopiraj">
                    <FiCopy />
                  </button>
              ...
              <div key={s.id} className="session-card">
                <div className="session-card-left">
                  ...
                  <div className="session-card-info">
                    ...
                    <span className="session-card-exercises">
                      ...
                      {s.status === 'in_progress' && ` · ${s.completed_exercises || 0}/${s.total_exercises} završeno`}
                    </span>
                  </div>
                </div>
                <div className="session-card-actions">
                  ...
                  {s.status === 'in_progress' && (
                    <button className="btn btn-sm btn-primary" onClick={() => navigate(`/plans/session/${s.id}`)}>
                      <FiPlay /> Nastavi
                    </button>
                  )}
                  {s.status === 'completed' && (
                    <button className="btn btn-sm btn-ghost" onClick={() => navigate(`/plans/session/${s.id}/detail`)}>
                      <FiEye /> Detalji
                    </button>
                  )}
                  {s.status !== 'completed' && (
