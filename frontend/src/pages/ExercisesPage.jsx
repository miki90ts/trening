...
function ExercisesPage() {
  const { exercises, loading, addExercise, editExercise, removeExercise, addCategory } = useApp();
  ...
  const [catForm, setCatForm] = useState({ name: '', value_type: 'reps', has_weight: false, description: '' });
  ...
  const openEdit = (ex) => {
    ...
    setForm({ name: ex.name, description: ex.description || '', icon: ex.icon || '💪' });
    setImageFile(null);
    setModalOpen(true);
  };
  ...
  const openAddCategory = (ex) => {
    ...
    setCatForm({ name: '', value_type: 'reps', has_weight: false, description: '' });
  ...
  return (
    <div className="page">
        ...
        {exercises.map(ex => (
          <Card key={ex.id} className="exercise-card">
                ...
                <ul>
                  {ex.categories.map(cat => (
                    <li key={cat.id}>
                      ...
                      {cat.has_weight ? <span className="cat-weight-badge">⚖️</span> : null}
                    </li>
                  ))}
                </ul>
              </div>
            ...
            <div className="card-actions">
              {isAdmin && (
                <>
                  <button className="btn btn-sm btn-secondary" onClick={() => openAddCategory(ex)}>
                    <FiPlus /> Kategorija
                  </button>
                  <button className="btn-icon" onClick={() => openEdit(ex)} title="Izmeni">
                    <FiEdit2 />
                  </button>
                  <button className="btn-icon btn-danger" onClick={() => handleDelete(ex)} title="Obriši">
                    <FiTrash2 />
                  </button>
                </>
              )}
      ...
      {/* Exercise modal */}
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editingExercise ? 'Izmeni vežbu' : 'Dodaj vežbu'}>
      ...
      {/* Category modal */}
      <Modal isOpen={catModalOpen} onClose={() => setCatModalOpen(false)} title={`Nova kategorija za: ${selectedExercise?.name}`}>
        <form onSubmit={handleAddCategory} className="form">
          ...
          <div className="form-group">
            ...
            <select value={catForm.value_type} onChange={e => setCatForm({ ...catForm, value_type: e.target.value })}>
          ...
          <div className="form-group">
            <label className="checkbox-label">
              <input
                ...
                onChange={e => setCatForm({ ...catForm, has_weight: e.target.checked })}
              />
              <span>Koristi tegove (kg) ⚖️</span>
            </label>
            <small className="form-hint">Uključite za vežbe sa tegom (bench press, squat, deadlift...)</small>
          </div>
          ...
          <button type="submit" className="btn btn-primary btn-full">Dodaj kategoriju</button>
        </form>
      </Modal>
    </div>
  );
