...
import {
  FiArrowLeft, FiCheck, FiCheckCircle, FiPlus, FiX,
  FiSave, FiChevronDown, FiChevronUp, FiTrash2, FiSearch
} from 'react-icons/fi';
...
function MealSessionExecutionPage() {
  ...
  const [newMacros, setNewMacros] = useState({ kcal: '0', protein: '0', carbs: '0', fat: '0' });

  useEffect(() => {
    api.getFoods({ limit: 9999 }).then(data => {
  ...
  const loadSession = useCallback(async () => {
    try {
      ...
      const expanded = {};
      data.meals?.forEach(m => { expanded[m.id] = true; });
  ...
  const toggleItem = (mealIdx, itemIdx) => {
    setSession(prev => {
      ...
      meal.is_completed = activeItems.length > 0 && activeItems.every(it => it.is_completed) ? 1 : 0;
  ...
  const handleAddCustomItem = (mealIdx) => {
    if (!newCustomName.trim()) { toast.warn('Unesite naziv stavke'); return; }
    setSession(prev => {
      const updated = { ...prev };
      const meals = [...updated.meals];
      const meal = { ...meals[mealIdx] };
      ...
      await api.updateMealSession(sessionId, {
        meals: session.meals.map(meal => ({
          ...
          items: meal.items.map(item => ({
            ...
            actual_amount_grams: item.actual_amount_grams != null ? parseFloat(item.actual_amount_grams) : null,
  ...
  const handleComplete = async () => {
    ...
    if (!anyCompleted) {
      toast.warning('Nema označenih stavki. Označi bar jednu stavku koju si pojeo.');
      return;
    }

    if (!window.confirm('Završi dan ishrane? Sve označene stavke će biti upisane u istoriju ishrane.')) return;
    ...
    try {
      ...
      toast.success(`${result.message} (${result.items_written} stavki upisano) 🍽️`);
      navigate('/meal-plans');
    } catch (err) {
    ...
    session.meals.forEach(m => {
      m.items.forEach(item => {
        if (item.is_completed && !item.is_removed) {
          const amt = item.actual_amount_grams != null ? parseFloat(item.actual_amount_grams) : parseFloat(item.amount_grams);
          totals.kcal += (amt / 100) * parseFloat(item.kcal_per_100g || 0);
          totals.protein += (amt / 100) * parseFloat(item.protein_per_100g || 0);
  ...
  return (
    <div className="page session-execution-page meal-session-page">
      ...
      <div className="session-exec-header">
        ...
        <div className="session-exec-title">
          ...
          <div className="session-exec-meta">
            <span className="session-progress-text">{stats.done}/{stats.total} stavki</span>
          </div>
        </div>
      ...
      <div className="meal-daily-totals-bar">
        <div className="meal-daily-total"><strong>{dailyTotals.kcal.toFixed(0)}</strong><small>kcal</small></div>
        <div className="meal-daily-total"><strong>{dailyTotals.protein.toFixed(1)}g</strong><small>Protein</small></div>
        <div className="meal-daily-total"><strong>{dailyTotals.carbs.toFixed(1)}g</strong><small>Ugljeni h.</small></div>
        <div className="meal-daily-total"><strong>{dailyTotals.fat.toFixed(1)}g</strong><small>Masti</small></div>
      </div>
      ...
      <div className="session-exercises">
        {session.meals.map((meal, mealIdx) => {
          ...
          const activeItems = meal.items.filter(it => !it.is_removed);
          const mealCompleted = activeItems.length > 0 && activeItems.every(it => it.is_completed);
          const removedItems = meal.items.filter(it => it.is_removed);

          // Meal totals (completed only)
          const mealTotals = activeItems.filter(it => it.is_completed).reduce((acc, item) => {
            const amt = item.actual_amount_grams != null ? parseFloat(item.actual_amount_grams) : parseFloat(item.amount_grams);
          ...
          return (
            <div key={mealKey} className={`session-exercise-card ${mealCompleted ? 'completed' : ''}`}>
              <div className="session-exercise-header" onClick={() => toggleExpand(mealKey)}>
                <div className="session-exercise-info">
                  ...
                  <div>
                    ...
                    <span className="session-exercise-cat">
                      {mealTotals.kcal.toFixed(0)} kcal · P: {mealTotals.protein.toFixed(0)}g · {activeItems.filter(it => it.is_completed).length}/{activeItems.length} stavki
                    </span>
                  </div>
                </div>
                <div className="session-exercise-right">
                  {mealCompleted && <FiCheckCircle className="exercise-done-icon" />}
                ...
                <div className="session-exercise-body">
                  ...
                  <div className="session-sets-table meal-items-table">
                    ...
                    {activeItems.map((item, rawIdx) => {
                      ...
                      const actualAmt = item.actual_amount_grams != null ? parseFloat(item.actual_amount_grams) : parseFloat(item.amount_grams);
                      const itemKcal = (actualAmt / 100) * parseFloat(item.kcal_per_100g || 0);

                      return (
                        <div key={item.id || `new_${rawIdx}`} className={`session-set-row ${item.is_completed ? 'completed' : ''}`}>
                          ...
                          <div className="form-group">
                            <input
                              ...
                              placeholder={String(parseFloat(item.amount_grams))}
                              value={item.actual_amount_grams != null ? item.actual_amount_grams : ''}
                              onChange={e => updateActualAmount(mealIdx, itemIdx, e.target.value)}
                              step="1"
                              min="0"
                            />
                          </div>

                          <span className="meal-item-kcal">{itemKcal.toFixed(0)}</span>
                  ...
                  {removedItems.length > 0 && (
                    <div className="meal-removed-items">
                      ...
                      {removedItems.map((item, i) => {
                        const itemIdx = meal.items.indexOf(item);
                        return (
                          <span key={i} className="meal-removed-badge" onClick={() => restoreItem(mealIdx, itemIdx)} title="Klikni da vratiš">
                  ...
                  {/* Add new item */}
                  {addingItem !== mealIdx ? (
                    <button className="btn btn-sm btn-ghost session-add-set" onClick={() => { setAddingItem(mealIdx); setFoodSearch(''); setNewCustomName(''); setNewAmount('100'); }}>
                      <FiPlus /> Dodaj stavku
                    </button>
                  ) : (
                    <div className="session-add-exercise-form card meal-add-item-form">
                      ...
                      <div className="food-search-input-wrapper" style={{ marginBottom: '0.5rem' }}>
                      ...
                      {foodSearch.length >= 2 && (
                        <div className="food-search-dropdown" style={{ position: 'relative' }}>
                          {foods.filter(f => f.name.toLowerCase().includes(foodSearch.toLowerCase())).slice(0, 8).map(f => (
                            <button
                              key={f.id}
                              type="button"
                            ...
                            >
                              <span className="food-option-name">{f.name}</span>
                      ...
                      <div className="meal-item-or-divider"><span>ili ručni unos:</span></div>

                      <div className="form-row" style={{ gap: '0.5rem', flexWrap: 'wrap' }}>
                        <div className="form-group" style={{ flex: 2, minWidth: '120px' }}>
                          <label>Naziv</label>
                          <input type="text" className="form-control" value={newCustomName} onChange={e => setNewCustomName(e.target.value)} placeholder="Naziv stavke" />
                        </div>
                        <div className="form-group" style={{ flex: 1, minWidth: '80px' }}>
                          <label>Grama</label>
                          <input type="number" className="form-control" value={newAmount} onChange={e => setNewAmount(e.target.value)} />
                        </div>
                        <div className="form-group" style={{ flex: 1, minWidth: '70px' }}>
                          <label>Kcal/100g</label>
                          <input type="number" className="form-control" value={newMacros.kcal} onChange={e => setNewMacros(p => ({ ...p, kcal: e.target.value }))} />
                        </div>
                        <div className="form-group" style={{ flex: 1, minWidth: '70px' }}>
                          <label>P/100g</label>
                          <input type="number" className="form-control" value={newMacros.protein} onChange={e => setNewMacros(p => ({ ...p, protein: e.target.value }))} />
                        </div>
                        <div className="form-group" style={{ flex: 1, minWidth: '70px' }}>
                          <label>C/100g</label>
                          <input type="number" className="form-control" value={newMacros.carbs} onChange={e => setNewMacros(p => ({ ...p, carbs: e.target.value }))} />
                        </div>
                        <div className="form-group" style={{ flex: 1, minWidth: '70px' }}>
                          <label>F/100g</label>
                          <input type="number" className="form-control" value={newMacros.fat} onChange={e => setNewMacros(p => ({ ...p, fat: e.target.value }))} />
                        </div>
                      </div>

                      <div className="form-actions">
                        <button className="btn btn-sm btn-primary" onClick={() => handleAddCustomItem(mealIdx)} disabled={!newCustomName.trim()}>
                          <FiPlus /> Dodaj
                        </button>
                        <button className="btn btn-sm btn-ghost" onClick={() => { setAddingItem(null); setFoodSearch(''); setNewCustomName(''); }}>
                          <FiX /> Otkaži
                        </button>
                      </div>
                    </div>
